...
import { fetchLatestChatHistory, sendAiChatMessage } from "../../services/api";
...
const CHAT_TEXT = {
  en: {
    ...
    aiUnavailable: "The AI backend is unavailable right now.",
    ...
    newChat: "New Chat",
    recent: "Recent",
    currentDraft: "Current draft chat",
  },
  om: {
    greeting:
      "Akkam! Ani gargaaraa kee dha. Har’a akkamitti si gargaaruu danda’a?",
    chat: "Haasawa",
    loadingConversation: "Walii haasawa fe'amaa jira...",
    noAiResponse: "Deebiin AI hin argamne.",
    aiUnavailable: "Tajaajilli AI yeroo ammaa hin jiruu.",
    placeholder: "Ergaa barreessi...",
    disclaimer: "AI dogoggora qabaachuu danda’a; deebii isaa mirkaneeffadhu.",
    history: "Seenaa",
    newChat: "Haasawa Haaraa",
    recent: "Dhihoo",
    currentDraft: "Haasawa ammaa",
  },
};

const buildDefaultGreeting = (content) => ({
  id: "greeting",
  role: "ai",
  content,
});

const mapBackendMessage = (message, index) => {
...
const Chat = () => {
  ...
  const text = CHAT_TEXT[language] || CHAT_TEXT.en;
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [messages, setMessages] = useState([
    buildDefaultGreeting(text.greeting),
  ]);
  ...
  useEffect(() => {
    ...
    const loadLatestHistory = async () => {
      try {
        const response = await fetchLatestChatHistory();
        ...
        setMessages(
          mapped.length > 0 ? mapped : [buildDefaultGreeting(text.greeting)],
        );
      } catch (_error) {
        if (!active) return;
        setMessages([buildDefaultGreeting(text.greeting)]);
      } finally {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const historyPayload = nextMessages.slice(-10).map((message) => ({
        role: message.role,
    ...
    <div className="flex h-full w-full bg-slate-50  text-slate-900 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 relative">
        ...
        <div className="md:hidden flex items-center justify-between p-4 border-b border-slate-200 bg-white">
          ...
          <h1 className="font-semibold text-slate-800">{text.chat}</h1>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 md:space-y-8">
              ...
              {messages.map((msg) => (
                ...
                >
                  ...
                  <div
                    className={`flex flex-col ${msg.role === "user" ? "items-end" : ""} min-w-0`}
                  >
                    <div
                      className={`px-4 md:px-5 py-2.5 md:py-3 rounded-2xl text-sm leading-relaxed ${
                        msg.role === "user"
      ...
      >
        <div className="p-4 flex flex-col h-full min-w-72">
          ...
          <button
            ...
            className="flex items-center gap-3 w-full p-3 bg-white border border-slate-200 rounded-xs font-medium hover:bg-slate-100 transition-all shadow-sm mb-6"
          >
            <Plus size={18} />
          ...
          <div className="flex-1 overflow-y-auto space-y-2">
            ...
            <div className="flex items-center gap-3 w-full p-3 text-slate-600 bg-white border border-slate-200 rounded-xl text-sm text-left">
