...
import { loginUser, saveStoredAuth } from "../services/api";
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const redirectMap = {
        student: "/dashbored",
        teacher: "/teacher-dashbored",
        admin: "/subscription-stats",
      };

      navigate(redirectMap[role] || "/dashbored");
    } catch (err) {
      ...
      <div className="w-full max-w-sm border border-gray-200 bg-white p-6">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2 bg-[#0056D2] text-white text-sm font-medium hover:bg-[#0045b0] transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
          >
